...
// The OAuthPersonalAccessTokenFunc type is an adapter to allow the use of ordinary
// function as OAuthPersonalAccessToken mutator.
type OAuthPersonalAccessTokenFunc func(context.Context, *ent.OAuthPersonalAccessTokenMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OAuthPersonalAccessTokenFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OAuthPersonalAccessTokenMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OAuthPersonalAccessTokenMutation", m)
}
